...
import { ContactDrawer } from '@/components/crm/views/contact-drawer'
import { CompanyDrawer } from '@/components/crm/views/company-drawer'
...
import { FileDrawer } from '@/components/crm/views/file-drawer'
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => !o && close()}>
      <SheetContent side="right" className="w-full sm:max-w-[640px] p-0 overflow-y-auto scroll-area">
        ...
        {!id && type === 'contact-new' && <ContactDrawer mode="create" onClose={close} />}
        {id && type === 'contact' && <ContactDrawer id={id} onClose={close} />}
        {!id && type === 'company-new' && <CompanyDrawer mode="create" onClose={close} />}
        {id && type === 'company' && <CompanyDrawer id={id} onClose={close} />}
        ...
        {id && type === 'file' && <FileDrawer id={id} onClose={close} />}
        {!id && type !== 'lead-new' && type !== 'contact-new' && type !== 'company-new' && type !== 'deal-new' && type !== 'task-new' && type !== 'note-new' && (
